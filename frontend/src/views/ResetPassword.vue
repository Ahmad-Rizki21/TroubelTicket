    ...
    <div class="login-form-wrapper">
      ...
      <div class="login-card">
        ...
        <div v-if="!token" class="error-message">
          <p>Token tidak ditemukan. Pastikan Anda mengakses halaman ini dari tautan reset password.</p>
          <p class="mt-2">
            <router-link to="/login" class="text-link">← Kembali ke Login</router-link>
          </p>
        </div>
        
        <!-- Reset Password Form -->
        <form v-if="token" @submit.prevent="handlePasswordReset" class="login-form">
...
onMounted(() => {
  ...
  if (tokenParam) {
    ...
    // If there's no token in the route, check the query string
...
const handlePasswordReset = async () => {
  if (!token.value) {
    errorMessage.value = 'Token tidak ditemukan. Tidak dapat mereset password.';
    return;
  }
  ...
  try {
    const response: any = await passwordResetAPI.resetPassword(
      token.value, // Use token from URL instead of username
      newPassword.value,
